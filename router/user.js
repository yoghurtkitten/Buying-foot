...
router.get("/getlist", (req, res) => {
  ...
  if (req.query.type) {
    ...
    sql =
      "SELECT * FROM shop WHERE province=? and city=? and county=? and shop_type=? and isPass=0 LIMIT ?,20";
  } else {
    sql =
      "SELECT * FROM shop WHERE province=? and city=? and county=? and isPass=0 LIMIT ?,20";
  }
...
router.get("/getFoods", (req, res) => {
  ...
  var sql =
    "SELECT * FROM shop JOIN food ON shop.id=food.shop_id WHERE food.shop_id=?";
...
router.get("/getFoodsCatagory", (req, res) => {
  ...
  var sid = req.query.sid;
  var sql =
    "SELECT type_name FROM shop JOIN food_catagory ON shop.id=food_catagory.shop_id WHERE shop.id=?";
  pool.query(sql, [sid], (err, result) => {
    if (err) {
      throw err;
    }
    if (result) {
      ...
      var sql2 =
        "SELECT * FROM shop JOIN food ON shop.id=food.shop_id JOIN food_catagory ON food.type_id=food_catagory.id WHERE food.shop_id=?";
...
router.post("/setShopCar", (req, res) => {
  ...
  pool.query(
    sql,
    [obj.user, parseInt(obj.foods_id), obj.sid, parseFloat(obj.un_price)],
    (err, result) => {
      if (err) {
        throw err;
      }
...
router.get("/load_shop_car", (req, res) => {
  var obj = req.query;
  var sql =
    "SELECT fid,food.name as name,number,un_price FROM shop_car JOIN food on shop_car.fid = food.food_id JOIN user ON shop_car.uid=user.id WHERE user.phone=? AND shop_car.isOrder=0";
...
router.post("/getOrderInfo", (req, res) => {
  ...
  var sql = `SELECT shop.shop_name,shop.deliver_fee,shop.deliver_time,food.name,shop_car.number,shop_car.un_price 
    FROM food JOIN shop_car ON food.food_id=shop_car.fid JOIN shop ON shop_car.shop_id=shop.id JOIN user ON shop_car.uid=user.id 
...
router.get("/valiAddress", (req, res) => {
  var obj = req.query;
  var sql =
    "select re_address.id, re_address.receiver,re_address.province,re_address.city,re_address.country,re_address.address,re_address.phone,re_address.gender from re_address JOIN user ON re_address.uid=user.id WHERE user.phone=? AND re_address.isDel=0";
  pool.query(sql, [obj.user], (err, result) => {
    if (err) {
      throw err;
    }
...
router.post("/save_Order", (req, res) => {
  ...
  var sql =
    "INSERT INTO order_ (uid, addr_id, shop_id, status, order_time,message,dish_count,price,order_no) VALUES ((SELECT id FROM user WHERE user.phone=?), ?, ?, 0, ?, ?, ?, ?, ?)";
  pool.query(
    sql,
    [
      obj.u_phone,
...
router.get("/change_order", (req, res) => {
  var obj = req.query;
  var sql =
    "UPDATE shop_car SET isOrder=? WHERE uid=(SELECT id FROM user WHERE user.phone=?) AND shop_id=? AND isOrder=0";
...
router.get("/getShopCarInfo", (req, res) => {
  var obj = req.query;
  var sql =
    "SELECT shop_car.number,shop_car.un_price*shop_car.number AS price,food.name FROM shop_car JOIN order_ ON shop_car.isOrder=order_.id JOIN food ON food.food_id=shop_car.fid WHERE order_.order_no=?";
  pool.query(sql, [obj.order_no], (err, result) => {
    if (err) {
      throw err;
    }
...
router.get("/getAddressInfo", (req, res) => {
  var obj = req.query;
  var sql =
    "SELECT re_address.receiver, re_address.address,re_address.phone,re_address.gender,order_.message,order_.order_time FROM order_ JOIN re_address ON order_.addr_id=re_address.id WHERE order_.order_no=?";
  pool.query(sql, [obj.order_no], (err, result) => {
    if (err) {
      throw err;
    }
...
router.post("/update_address", (req, res) => {
  var obj = req.body;
  var sql =
    "UPDATE re_address SET receiver=?, province=?, city=?, country=?, address=?, phone=?, gender=? WHERE id=?";
  pool.query(
    sql,
    [
      obj.receiver,
...
router.post("/SaveShop", (req, res) => {
  var obj = req.body;
  var sql =
    "INSERT INTO save VALUES(null, (SELECT id FROM user WHERE user.phone=?), ?, 0)";
...
router.get("/isSave", (req, res) => {
  var obj = req.query;
  var sql =
    "SELECT id, isDel FROM save WHERE uid=(SELECT id FROM user WHERE user.phone=?) AND sid=?";
  pool.query(sql, [req.session.user.name, obj.sid], (err, result) => {
    if (err) {
      throw err;
    }
...
router.get("/getSave", (req, res) => {
  var obj = req.query;
  var sql =
    "SELECT shop.id, shop.shop_name, shop.shop_start, shop.deliver_cost, shop.deliver_fee, shop.deliver_time, shop.shop_img FROM save JOIN shop ON save.sid=shop.id WHERE uid=(SELECT id FROM user WHERE user.phone=?)";
  pool.query(sql, [req.session.user.name], (err, result) => {
    if (err) {
      throw err;
    }
...
router.get("/getOrderStatus", (req, res) => {
  var obj = req.query;
  var sql = "SELECT status,comment,order_star,busi_comment FROM order_ WHERE order_no=?";
  pool.query(sql, [obj.order_no], (err, result) => {
    if (err) {
      throw err;
    }
...
router.get("/hasFood", (req, res) => {
  var obj = req.query;
  var sql =
    "SELECT COUNT(food.food_id) as c FROM shop JOIN food ON shop.id=food.shop_id WHERE shop.id=?";
  pool.query(sql, [obj.sid], (err, result) => {
    if (err) {
      throw err;
    }
...
router.get("/getFoodCount", (req, res) => {
  ...
  var sql =
    "UPDATE food SET inventory=inventory-? WHERE food_id=? AND inventory>?";
...
router.get("/getInfoToComment", (req, res) => {
  var obj = req.query;
  var sql = `SELECT order_time,shop.shop_name,shop.shop_img,food.name,shop_car.number FROM 
    order_ JOIN shop ON order_.shop_id=shop.id JOIN shop_car ON shop_car.isOrder=order_.id JOIN 
    food ON shop_car.fid=food.food_id WHERE order_.order_no=?`;
  pool.query(sql, [obj.order_no], (err, result) => {
    if (err) {
      throw err;
    }
...
router.post("/setUserComment", (req, res) => {
  var obj = req.body;
  var sql = `UPDATE order_ SET comment=?,order_star=? WHERE order_no=?`;
  pool.query(
    sql,
    [obj.comment, obj.order_star, obj.order_no],
    (err, result) => {
      if (err) {
        throw err;
      }
...
router.get("/getUserComment", (req, res) => {
  var obj = req.query;
  var sql = `SELECT user.phone,order_.order_no,order_.comment,order_.order_star,order_.busi_comment FROM order_ JOIN 
    user ON order_.uid=user.id JOIN shop ON order_.shop_id=shop.id JOIN business ON 
    shop.business_id=business.id WHERE order_.comment <> '' AND business.phone=?`;
...
router.post("/setBusiComment", (req, res) => {
  var obj = req.body;
//   console.log(obj)
  var sql = `UPDATE order_ SET busi_comment=? WHERE order_.order_no=?`;
  pool.query(sql, [obj.comment, obj.order_no], (err, result) => {
    if (err) {
      throw err;
    }
    if (result.affectedRows) {
